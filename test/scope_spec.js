...
describe('Scope', function () {
    ...
    describe('digest', function () {
        ...
        it('calls the watch function with the scope as the argument', function () {
            ...
            /*
             * watchFn is a spy. we want to know what
             * arguments were used as parameters.
             * 
             * this is because watchFn needs to be 
             * called with scope to support dirty
             * checking.
             */
            ...
            scope.$watch(watchFn, listenerFn);
            scope.$digest();
            expect(watchFn).toHaveBeenCalledWith(scope); // as an argument
        });
        ...
        it('allows a $watch to destroy another during digest 957', function () {
            scope.aValue = 'abc';
            scope.counter = 0;
            scope.$watch(
                function (scope) {
    ...
    describe('inheritance: ', function () {
        ...
        it('keeps a record of its children', function () {
            ...
            var grandchild = parent.$new();
            var grandchild_1 = grandchild.$new();
            expect(parent.$$children.length).toBe(2);
            expect(parent.$$children[0]).toBe(child1);
            expect(parent.$$children[1]).toBe(grandchild);
            expect(child1.$$children.length).toBe(0);
            expect(grandchild.$$children.length).toBe(1);
            expect(grandchild.$$children[0]).toBe(grandchild_1);
        });
    ...
    it('schedules a digest from root on $evalAsync', function (done) {
        ...
        grandchild.$evalAsync(function (scope) { });
        setTimeout(function () {
            expect(parent.counter).toBe(1);
            done();
        }, 50);
    ...
    it('schedules a digest from root on $evalAsync when isolated', function (done) {
        var parent = new Scope();
        var isolatedchild = parent.$new(true);
        var grandchild = isolatedchild.$new();
        parent.aValue = 'abc';
        parent.counter = 0;
        parent.$watch(
            function (scope) { return scope.aValue; },
    ...
    describe('$watchCollection: ', function () {
        ...
        it('notices an item replaced in an (array-like) arguments object', function () {
            (function () {
